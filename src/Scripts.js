function initialize() {
  ...
  summarySheet.insertRowAfter(lastRow);
  
  var newRowRange = summarySheet.getRange(lastRow + 1, 1, 1, summarySheet.getLastColumn());
  
  newRowRange.setBackground('white');
  
  var mergedRange = summarySheet.getRange(lastRow + 1, 1, 1, 5);
  mergedRange.merge();
  
  summarySheet.setRowHeights(lastRow + 1, 1, 560);


  var offsetX = 0;
  summarySheet.getCharts()
...
function createTriggers() {
  ...
  ScriptApp.newTrigger('updateSummaryData')
    .timeBased().atHour(1)
    .everyDays(1)
    .create();
}
